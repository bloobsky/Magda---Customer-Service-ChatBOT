...
class ChatBot:
    ...
    def load_data(self, filename):
        ...
        # Lemmatize (group together), sorting and lower each word
        # and remove duplicates or remove ignore_words
        self.words = sorted(list(set([self.lemmatizer.lemmatize
                            (to_word.lower()) for to_word
                            in self.words if to_word
                            not in self.ignore_words])))
        # Sorting Classes
        self.classes = sorted(list(set(self.classes)))

        # Pickle module is responsible for transfering string into a binary
        # code that could be understand by computer
    ...
    def create_training_data(self):
        # It's a collection of words to represent a sentence with word count
        #  and mostly disregarding the order in which they appear
        training = []
        ...
        for doc in self.documents:
            ...
            pattern_words = [self.lemmatizer.lemmatize(word.lower())
                             for word in pattern_words]
            # Creating BOW (Bag of Words) array wwith 1 when word match
        ...
        # Shuffle data and putting it inside numpy array which is used
        # for scientific data, has to have dtype=object
    ...
    def create_model(self):
        ...
        # that are equal to number of intents to
    ...
    def train_model(self, epochs, batch_size):
        # Fit the model
        self.hist = self.model.fit(np.array(self.train_x),
